...
# Brainstorm Pipeline
# 27 timepoints = 27 FCS files
# For loop with 27 iterations eg. For file in fileList
# inside the ForLoop have it to the following: 
    # 1) read in file and make into gating set with timepoint number as the object name
              # requires interactively setup of experiment marker and experiment details.csv files? Is there a way to NOT use the interactive function. Have the code write the csv files? 
    # 2) multiple steps transformation  -- write it as a function have it return the final transformed gating set
    # 3) 
...
#For Loop to Make Gating Sets and Experiment CSV files 
      # I don't like that it asks us to 'Save & Close' the experiment csv files in each iteration of the loop...or we only need the FIRST iteration's csv files. Someway to stop it in the next iterations? Nice thing is it doesn't overwrite the csv files int he next iteration. It detects the firdt one and keeps it .
# have unique experiment details.csv file for each. rename them in the function. 

#apply function with mapping purr instead of for loop -- because it will run in parallel. 
for (file in (listFiles)){
  #print(file)
  tp_number = str_sub(file, 93, 94) #extract timepoint number
  #print(tp_number)
  assign(paste("timept",tp_number,"_gs", sep = ""), cyto_setup(path = file,restrict = TRUE, select = "fcs")) #make a gating set per file
}
#Error in paste("timept", tp_number, "_gs", sep = "") = cyto_setup(path = file,  : target of assignment expands to non-language object
  #trying to write new name like this does not work: "timepoint"+(str_sub(file, 1, 2))+"gs"= cyto_setup(path = file,restrict = TRUE, select = "fcs")

  #extract the timepoint info from the filename ie) first two positions
  #set the timepoint info as name of new the gating set ie) timept02_gating_set
...
# Edit the experimental details with columns containing A-F 
# ALSO rename the .csv file with the unique folder name name
# Universal experimental-markers.csv file (same for all timepoints)
...
cyto_plot_gating_scheme(transformed_time01[length(transformed_time01)-1], #check % of DGY1 control cells that fit in the zero_copy gate
                        back_gate = TRUE,
                        gate_track = TRUE)
cyto_plot_gating_scheme(transformed_time01[1], #check % most of DGY500 control cells that fit in the one_copy gate
                        back_gate = TRUE,
                        gate_track = TRUE)
