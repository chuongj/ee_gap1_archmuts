...
########## Zoom into generation 8-50 ################
early = freq_and_counts %>%
  filter(Count>70000,
         generation <= 50) %>%
  filter(Gate %in% c("two_or_more_copy"), Type == "Experimental") %>%
  anti_join(fails)  %>% #remove contaminated and outliers informed by population ridgeplots (above) and fluor lineplots (below)
...
#ggsave("propCNV_early.pdf", plot = early, width = 12, height = 12)
#ggsave("propCNV_early.png", plot = early, width = 12, height = 12)


## make and apply new gates for gens 8 and 21 ?? keep the gates after g21 the as-is gates.
# g8, g21, g29.
# at g8 and g21 we seem to have one copy experimental strain going into the CNV gate...
...
View(early_df)
#write_csv(x = early_df, file = "propCNV_early.csv")
