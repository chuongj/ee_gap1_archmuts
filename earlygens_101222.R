...
#folders[3] Choose the 01_02_04_EE_GAP1_ArchMuts_2021_unrenamed folder
exp_details_1 = list.files(path = paste0(folders[7]), pattern = "_experiment_details.csv", full.names = T)
exp_details_2 = list.files(path = paste0(folders[8]), pattern = "_experiment_details.csv", full.names = T)
exp_details_3 = list.files(path = paste0(folders[9]), pattern = "_experiment_details.csv", full.names = T)

t1 <- cyto_setup(path = paste0(folders[7]), restrict=TRUE, select="fcs", details=F) #edit Markers on Viewer pane, Save & Close
t2 <- cyto_setup(path = paste0(folders[8]), restrict=TRUE, select="fcs", details=F)
t3 <- cyto_setup(path = paste0(folders[9]), restrict=TRUE, select="fcs", details=F)

#use flowWorkspace::pData to annotate the experiment details file associated with the gating set
d1 <- read_csv(exp_details_1) #import experiment-details.csv
d2<- read_csv(exp_details_2)
d3<- read_csv(exp_details_3)

for(i in 1:length(names(d1))){
  flowWorkspace::pData(t1)[names(d1[i])]<-d1[i]
}
cyto_details(t1)

for(i in 1:length(names(d2))){
  flowWorkspace::pData(t2)[names(d1[i])]<-d2[i]
}
cyto_details(t2)

for(i in 1:length(names(d3))){
  flowWorkspace::pData(t3)[names(d1[i])]<-d3[i]
}
cyto_details(t3)
...
GFP_trans <- cyto_transformer_logicle(t1,
                                      channels = c("B2-A"),
                                      widthBasis = -10
)#returns it as a list
FSC_SSC_trans <- cyto_transformer_log(t1,
                                      channels = c("FSC-A", "FSC-H", "SSC-A", "SSC-H")
)
combined_trans <- cyto_transformer_combine(GFP_trans,FSC_SSC_trans)
transformed_t1 <- cyto_transform(t1,
