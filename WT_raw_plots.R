#Purpose: Look at the raw data for the Wildtype evolution experiments.  I think this would be best displayed as the 2D plots (FSC vs GFP) for each time point in a time series. If David and I could look at them together we can make sure we understand the sources of variation in the data and think about how to explain the difference in results with prior experiments.
...
timepoint_gating_set <- cyto_setup(path = paste0(input_dir), restrict=TRUE, select="fcs", details=F) #edit Markers on Viewer pane, Save & Close
