...
library(CytoExploreR)  #it loads flowCore and flowWorkspace too
...
timept01_gating_set <- cyto_setup(path="~/nyudrive/greshamlab/projects/EE_GAP1_ArchMuts_Summer2021/data/FCS_files/01_EE_GAP1_ArchMuts_2021_061621_g8_GA", select="fcs")
...
cyto_plot_profile(timept01_gating_set,
                  parent = "root",
                  channels = c("FSC-A","GFP")
) #two expression profiles ridgeplots with % of mode of y-axis, FSC-A and GFP on x-axis. we chose the channels to display 
...
accuri_transformed <- cyto_transformer_logicle(timept01_gating_set)
